...
def init_db():
    ...
    c.execute("INSERT OR IGNORE INTO bot_settings (setting_key, setting_value) VALUES ('timer_seconds', '60')")
    c.execute("INSERT OR IGNORE INTO bot_settings (setting_key, setting_value) VALUES ('message1_text', '✅ Your file has been sent.\n\n⏰ This message will be deleted in {timer} seconds.\n\n📌 To save permanently: Forward this message to Saved Messages or your channel/group.')")
    c.execute("INSERT OR IGNORE INTO bot_settings (setting_key, setting_value) VALUES ('message2_text', '📝 Video Player Info:\n\n📱 Android: MX Player, VLC Player\n🍎 iPhone: VLC for Mobile, PlayerXtreme\n💻 Windows/Mac: VLC Media Player\n\nDownload and play with any player above.\n\n━━━━━━━━━━━━━━━━━━\n📢 Admin: {admin_username}')")
    ...
    c.execute("INSERT OR IGNORE INTO ads_settings (id, is_enabled, daily_limit) VALUES (1, 1, 3)")
...
def record_ad_view(user_id, short_code):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("INSERT INTO ad_views (user_id, short_code) VALUES (?, ?)", (user_id, short_code))
    conn.commit()
    conn.close()
...
def save_batch(batch_code, chat_id, total_files):
    ...
    c.execute("INSERT INTO batches (batch_code, chat_id, total_files) VALUES (?, ?, ?)",
              (batch_code, chat_id, total_files))
