...
async def delete_message_after_delay(chat_id, message_id, delay_seconds):
    ...
    try:
        context = await Application.builder().token(BOT_TOKEN).build()
        await context.bot.delete_message(chat_id=chat_id, message_id=message_id)
        logger.info(f"Deleted message {message_id} after {delay_seconds}s")
    except Exception as e:
        logger.error(f"Failed to delete: {e}")

async def send_with_auto_delete(update, text, delay_seconds, reply_markup=None):
    ...
    task = asyncio.create_task(delete_message_after_delay(update.effective_chat.id, msg.message_id, delay_seconds))
    delete_tasks[msg.message_id] = task
        ...
        if code.startswith("file_"):
            ...
            if file_data:
                ...
                if not ads_enabled or is_bypass or has_viewed_ad(user_id, short_code):
                    ...
                    # Send Message 1 with auto delete
                    timer = get_timer()
                    msg_text = get_message1().format(timer=timer)
                    keyboard = [[InlineKeyboardButton("📝 Player Info", callback_data="player")]]
                    await send_with_auto_delete(update, msg_text, timer, InlineKeyboardMarkup(keyboard))
                else:
            ...
            if batch_files:
                ...
                if not ads_enabled or is_bypass or has_viewed_ad(user_id, batch_code):
                    ...
                    await send_with_auto_delete(update, msg_text, timer)
                else:
                    ...
                    await update.message.reply_text(f"📦 {len(batch_files)} files ready! Watch 1 ad:", reply_markup=InlineKeyboardMarkup(keyboard))
            else:
...
async def genlink(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "📤 **Send files**\nType /done when finished, /cancel to stop.",
        parse_mode='Markdown'
    )
...
async def genlink_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(files) == 1:
        ...
        await update.message.reply_text(
            f"✅ **Batch Link:**\n`{link}`\n\n📁 {len(files)} files, watch 1 ad to get all.",
            parse_mode='Markdown'
        )
...
async def ads_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"**Ads:** {status}\n**Daily Limit:** {limit}\n**Bypass:** {bypass_count}", parse_mode='Markdown')
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin_user:
        text += "/genlink, /done, /cancel\n"
        text += "/set_timer, /set_admin_username, /set_message1, /set_message2, /show_messages\n"
        text += "/ads_on, /ads_off, /ads_status, /set_ad_limit, /add_bypass, /remove_bypass\n"
        text += "/add_channel, /remove_channel, /channels\n"
        text += "/admins\n"
    
    if user_id == OWNER_ID:
        text += "/addadmin, /removeadmin\n"
...
def main():
    ...
    logger.info("✅ Bot started with Auto Delete working!")
    print("✅ Bot started with Auto Delete working!")
