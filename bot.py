    ...
    if context.args:
        ...
        if file_code.startswith("file_"):
            ...
            if file_data:
                ...
                # Check if ads are enabled and user has not viewed ad
                ...
                if not ads_enabled or is_bypass or has_viewed_ad(user_id, short_code):
                    ...
                    # Send confirmation with player info button
                    timer = get_timer()
...
def main():
    ...
    # Message handlers - FIXED: removed DOCUMENT, using Document.ALL
    app.add_handler(MessageHandler(filters.VIDEO | filters.PHOTO | filters.Document.ALL | filters.AUDIO, handle_file))
    app.add_handler(MessageHandler(filters.VIDEO | filters.PHOTO | filters.Document.ALL | filters.AUDIO, handle_batch_file))
