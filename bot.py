# bot.py - FULL FIXED (Ads for both single and batch)
import asyncio
import threading
import json
import logging
        ...
        if file_code.startswith("file_"):
            ...
            if file_data:
                if has_viewed_ad(user_id, short_code):
                    await send_file_by_type(update, file_data[0], file_data[1])
                else:
                    ...
                    await update.message.reply_text("Watch ad to get file:", reply_markup=InlineKeyboardMarkup(keyboard))
            else:
            ...
            if batch_files:
                # Check if user viewed ad for this batch
                if has_viewed_ad(user_id, batch_code):
                    ...
                    await update.message.reply_text(f"📦 {len(batch_files)} files ready. Watch ad once to get all:", reply_markup=InlineKeyboardMarkup(keyboard))
            else:
...
async def genlink(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_files[user_id] = []
    await update.message.reply_text("Send files. Type /done when finished, /cancel to stop.")
...
async def handle_genlink_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ File #{len(user_files[user_id])} added. Send more or /done")
...
async def genlink_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(files) == 1:
        ...
        c = conn.cursor()
        c.execute("INSERT INTO batches (batch_code, chat_id, total_files) VALUES (?, ?, ?)", (batch_code, update.effective_chat.id, len(files)))
        ...
        await update.message.reply_text(f"✅ **Batch Link:**\n`{link}`\n\n📁 {len(files)} files, watch ad once to get all.", parse_mode='Markdown')
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == OWNER_ID:
        text += "/addadmin <id>\n/removeadmin <id>\n"
    text += "\n**Users:** Click link → Watch ad → Get file(s)"
    await update.message.reply_text(text, parse_mode='Markdown')
...
def main():
    ...
    logger.info("Bot started! Ads working for single and batch.")
    app.run_polling()
