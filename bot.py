        ...
        if code.startswith("file_"):
            ...
            if file_data:
                ...
                if not ads_enabled or is_bypass or has_viewed_ad(user_id, short_code):
                    ...
                    keyboard = [[InlineKeyboardButton("📝 Player Info", callback_data=f"player")]]
                    ...
                    await update.message.reply_text("Click to get your file:", reply_markup=InlineKeyboardMarkup(keyboard))
            else:
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    text = "**📖 Bot Commands V2 (Complete):**\n\n"
    
    if is_admin(user_id):
        text += "**📁 File:**\n"
        text += "/genlink - Create link (1 or multiple files)\n"
        text += "/done - Finish file collection\n"
    ...
    text += "**Users:** Click link → Watch ad → Get file(s)"
