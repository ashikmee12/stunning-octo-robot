...
# Native auto delete function
async def auto_delete_message(message, delay_seconds):
    """Delete message after delay"""
    await asyncio.sleep(delay_seconds)
    try:
        await message.delete()
        logger.info(f"Deleted message {message.message_id} after {delay_seconds}s")
    except Exception as e:
        ...
        if code.startswith("file_"):
            ...
            if file_data:
                ...
                if not ads_enabled or is_bypass or has_viewed_ad(user_id, short_code):
                    ...
                    msg = await update.message.reply_text(msg_text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
                    asyncio.create_task(auto_delete_message(msg, timer))
                else:
...
def main():
    ...
    logger.info("🤖 Bot started with Native Auto Delete working!")
    print("🤖 Bot started with Native Auto Delete working!")
