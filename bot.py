...
async def genlink(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Type **/cancel** to stop.\n\n"
        "💡 1 file = single link\n"
        "💡 Multiple files = batch link (watch 1 ad to get all)",
        parse_mode='Markdown'
    )
...
async def set_timer_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        await update.message.reply_text(f"Current timer: {current} seconds ({current_hours} hours)\n\nUsage: /set_timer <seconds>\nExample: /set_timer 43200 (12 hours)")
        return
    try:
        seconds = int(context.args[0])
        if seconds < 60:
            await update.message.reply_text("Minimum 60 seconds.")
            return
        set_timer(seconds, update.effective_user.id)
        hours = seconds / 3600
        await update.message.reply_text(f"✅ Timer set to {seconds} seconds ({hours} hours).")
    except:
...
async def show_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"**📊 Current Settings:**\n\n"
        f"**Timer:** {timer} seconds ({timer_hours} hours)\n"
...
async def set_ad_limit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if limit < 1:
            ...
            return
        set_ad_limit(limit)
        await update.message.reply_text(f"✅ Daily ad limit set to {limit}.")
    except:
        await update.message.reply_text("Invalid number.")
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text += "1. `/genlink` - Start collecting\n"
...
def main():
    ...
    logger.info("🤖 Bot started with ALL features! Timer: 12 hours (43200 seconds)")
    print("🤖 Bot started with ALL features! Timer: 12 hours (43200 seconds)")
